...
{
    public class clsRetailBuyerOrder
    {
        ...
        public clsRetailBuyerOrder(Boolean bAllowNegative, String _LoggedLocation, String _LoggedUserCode, String _LoggedUserAlias, String _LoggedBin)
        {
        ...
        public String SaveRetailBuyerOrder(Int32 iMode, String cMemoID, String cConStr, String LoggedLocation, String LoggedUserCode, String LoggedUserAlias, DataTable tCM_MST, DataTable tCM_DET, out DataTable dtResult)
        {
            String cRetVal = "";
            ...
            if (iMode == 1)
            {
                cRetVal = AddRetailBuyerOrder(cConStr, LoggedLocation, LoggedUserCode, LoggedUserAlias, tCM_MST, tCM_DET, out dtResult);
            }
            else if (iMode == 2)
            {
                cRetVal = UpdateRetailBuyerOrder(cConStr, LoggedLocation, LoggedUserCode, LoggedUserAlias, tCM_MST, tCM_DET, out dtResult);
            }
            else if (iMode == 3)
            {
                cRetVal = CancelRetailBuyerOrder(cConStr, LoggedLocation, LoggedUserCode, LoggedUserAlias, cMemoID, out dtResult);
            }
            return cRetVal;
        }

        public String AddRetailBuyerOrder(String cConStr, String LoggedLocation, String LoggedUserCode, String LoggedUserAlias, DataTable tCM_MST, DataTable tCM_DET, out DataTable dtResult)
        {
            dtResult = new DataTable("TDATA");
            dtResult.Columns.Add("ERRMSG", typeof(System.String));
            dtResult.Columns.Add("MEMO_ID", typeof(System.String));
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(cConStr))
                {
                    ...
                    DateTime dtCMDT = globalMethods.ConvertDateTime(tCM_MST.Rows[0]["order_dt"]);
                    ...
                    try
                    {
                        do
                        {
                            cQueryStr = @"
                                ...
                                EXEC GETNEXTKEY_OPT @CTABLENAME='WSL_ORDER_MST', @CCOLNAME='ORDER_NO',@NWIDTH= 10, @CPREFIX='" + cPrefix + @"', @NLZEROS=1,@CFINYEAR='011" + cFinYear + @"' ,@NROWCOUNT=0,@CKEYSTABLE='" + CKEYSTABLE + @"',@CNEWKEYVAL= @CMEMONOVAL OUTPUT
                            ...
                            {
                                cQueryStr = @"SELECT COUNT(*) FROM WSL_ORDER_MST(NOLOCK) 
					                WHERE ORDER_NO='" + cMemoNo + @"' AND FIN_YEAR = '011" + cFinYear + @"' ";
                        ...
                        //if (sbInsertDetValues.Length > 0)
                        {
                            ...
                            //cmd.CommandType = CommandType.Text;
                            //cmd.ExecuteNonQuery();
                            tCM_DET.Select("").ToList<DataRow>().ForEach(r => r["ORDER_ID"] = cMemoID);
                            tCM_DET.Select("").ToList<DataRow>().ForEach(r => r["ROW_ID"] = GetNewRowID(LoggedLocation));

                        }
                        ChangeDBNull(tCM_MST, "WSL_ORDER_MST");
                        ChangeDBNull(tCM_DET, "WSL_ORDER_DET");
                        ...
                        using (SqlBulkCopy bulkCopy = new SqlBulkCopy(con, options, sqlTran))
                        {
                            ...
                            bulkCopy.DestinationTableName = "WSL_ORDER_MST";
                            globalMethods.SelectCmdToSql(dset, "SELECT * FROM WSL_ORDER_MST(NOLOCK) WHERE 1=2", "TCURSOR_CMM");
                        ...
                        using (SqlBulkCopy bulkCopy = new SqlBulkCopy(con, options, sqlTran))
                        {
                            bulkCopy.BulkCopyTimeout = 50000;
                            bulkCopy.BatchSize = 5000;
                            bulkCopy.DestinationTableName = "WSL_ORDER_DET";
                            globalMethods.SelectCmdToSql(dset, "SELECT * FROM WSL_ORDER_DET(NOLOCK) WHERE 1=2", "TCURSOR_CMD");
              ...
              //                  AS
              //                  (
              //                      SELECT PRODUCT_CODE,BIN_ID,LEFT('" + cMemoID + @"' ,2) AS DEPT_ID,SUM(QUANTITY) AS QUANTITY 
              //                      from WSL_ORDER_DET(NOLOCK) 
              //                      WHERE ORDER_ID='" + cMemoID + @"'
              //                      GROUP BY PRODUCT_CODE,BIN_ID
              //                  )
        ...
        public String UpdateRetailBuyerOrder(String cConStr, String LoggedLocation, String LoggedUserCode, String LoggedUserAlias, DataTable tCM_MST, DataTable tCM_DET, out DataTable dtResult)
        {
            dtResult = new DataTable("TDATA");
            dtResult.Columns.Add("ERRMSG", typeof(System.String));
            dtResult.Columns.Add("MEMO_ID", typeof(System.String));
            ...
            try
            {
                ...
                using (SqlConnection con = new SqlConnection(cConStr))
                {
                    ...
                    cMemoID = Convert.ToString(tCM_MST.Rows[0]["ORDER_ID"]);
            ...
            //            cmd.ExecuteNonQuery();
                        ...
                        cQueryStr = @"UPDATE A SET ORDER_ID='XXXXXXXXXX'
                                    from WSL_ORDER_DET A WITH (ROWLOCK)
                                    WHERE ORDER_ID='" + cMemoID + "'";
                        ...
                        cQueryStr = @"Update A SET 
                            ...
                            from WSL_ORDER_MST A WITH (ROWLOCK)
                            WHERE ORDER_ID='" + cMemoID + "'";
        ...
        public String CancelRetailBuyerOrder(String cConStr, String LoggedLocation, String LoggedUserCode, String LoggedUserAlias, String MemoID, out DataTable dtResult)
        {
            dtResult = new DataTable("TDATA");
            dtResult.Columns.Add("ERRMSG", typeof(System.String));
            dtResult.Columns.Add("MEMO_ID", typeof(System.String));
                ...
                using (SqlConnection con = new SqlConnection(cConStr))
                {
                    ...
                    if (!_bAllowNegative)
                    {
                        cQueryStr = @"
                                ...
                                (
                                    SELECT A.PRODUCT_CODE,A.BIN_ID,c.location_code AS DEPT_ID,SUM(A.QUANTITY) AS QUANTITY 
                                    from RPS_DET A(NOLOCK) JOIN RPS_MST C (NOLOCK) ON C.CM_ID=A.CM_ID
